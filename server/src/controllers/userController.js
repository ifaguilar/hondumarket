...
export const addUserRating = async (req, res, next) => {
    ...
    if (!description || !rateNumber) 
      throw new Error("Es necesario ingresar una calificacion y una descripcion")
...
export const addUserComplaint = async (req, res, next) => {
    ...
    if (!description || !complaintCategoryID) 
      throw new Error("Es necesario ingresar una categoria y una descripcion")
...
export const changeUserStatus = async (req, res) => {
	...
	try {
      ...
      `, [isActive, userID]
		...
		if (resDB.rowCount < 1)
			throw new Error("Algo salio mal y no se pudo actualizar el estado del usuario");
